...
- 管理者は図書館員に権限譲渡可（前管理者は図書館員に降格される）。常に1名。
- 降格された管理者は特定期間内であれば管理者に復帰可（緊急時）。その場合は新管理者は自動的に図書館員となる
- 一般ユーザー と 図書館員は相互に昇格・降格可。昇格時には管理者または現図書館員の操作を必要とする。
- 図書館員の一般ユーザーへの降格は自身でも可能。
- 管理者 → 一般ユーザー：不可。
...
- users.csv が存在しない場合等新規起動と見做し「初期起動モード」となる
- 「初期起動モードであれば」初期管理者：`USR00001` / `admin12345`を引数として login.pyに渡す。
- `--reset-admin` により管理者パスワードをリセット可（他データは保持）。
- リセットオプションをつけて起動された場合にも初期管理者アカウント及びパスワードを引数として login.py に渡す。
...
- 「初期起動モード」・パスワードリセット時別プログラム(system_boot.py)から受けた引数に応じた初期IDとパスワードを表示。
- 上記の場合はログイン成功後、必須としてパスワード変更（change_pass.py）を促す。
...
| Fキー | 機能 | モジュール | 備考 |
|-------|------|------------|------|
| F1 | 貸出 | borrow.py | 利用者ID → 書籍ID。Uキー押下でユーザー検索画面に遷移(.py)。条件によりユーザー検索→戻り値で得た利用者IDを利用 |
| F2 | 返却 | return_book.py | 書籍IDを複数入力 |
| F3 | 書籍検索 | search.py | 書籍を条件検索 |
| F4 | ユーザー検索 | user_search.py | 管理者・図書館員用全機能 (利用者には未開放)|
| F8 | システム情報 | 未実装 | 書籍数、利用者数等(管理者及び図書館員のみ)|
| F12 | ログアウト | logout.py | ログアウト処理後、ログインメニューへ戻る |
